...
launch_sites_options = [{'label': 'All Sites', 'value': 'ALL'}] + [{'label': site, 'value': site} for site in launch_sites]
...
def get_pie_chart(entered_site):
    if entered_site == 'ALL':
        grouped_df = spacex_df.groupby('Launch Site')['class'].value_counts().unstack().fillna(0)
        grouped_df.reset_index(inplace=True)
        
        fig = px.pie(grouped_df, 
                     names='Launch Site',  # Use launch sites as names
                     values=1,  # Use 'Success' counts as values
...
def get_scatter_chart(entered_site, selected_payload):
    if entered_site == 'ALL':
        ...
        fig = px.scatter(filtered_df,
                         ...
                         title=f'Successful Launches Scatter plot for {entered_site}')
        return fig
    else:
        # Filter the dataframe based on the payload range and selected launch site
        filtered_df = spacex_df[(spacex_df['Payload Mass (kg)'] >= selected_payload[0]) &
                                (spacex_df['Payload Mass (kg)'] <= selected_payload[1]) &
